...
**What data is required:** A stable actor identifier per session; the raw principal used on each surface (API key, OAuth subject, service account); a mapping from those principals to a canonical human and org identity; the delegation / on-behalf-of chain (human -> agent -> sub-agent -> tool); and organizational metadata (team, cost center) attached to the resolved identity.
